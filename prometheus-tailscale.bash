...
# ==============================================================================
# Fast Prometheus + Tailscale + VictoriaMetrics Agent + Firewall + BBR Setup
# ==============================================================================
...
# Запрос URL с дефолтным значением Tailscale
DEFAULT_URL="http://100.105.78.126:8428/api/v1/write"
if [ -c /dev/tty ]; then
...
echo "Кликните по ней и авторизуйтесь в браузере."
echo "------------------------------------------------------------------"
tailscale up || true
...
# Настройка Белого Списка (Whitelist): Tailscale, Localhost, локальные подсети, SSH и домены
...
# 3. Разрешаем интерфейс Tailscale
if ! iptables -C INPUT -i tailscale0 -j ACCEPT 2>/dev/null; then
    iptables -I INPUT 3 -i tailscale0 -j ACCEPT
fi

# 4. Разрешаем весь Whitelist (включая CGNAT Tailscale 100.64.0.0/10, доверенные IP и домены)
...
# 6. Ограничиваем порт 2222 (разрешен только для белого списка/Tailscale/localhost, для остальных DROP)
...
echo "   - Ограничение порта 2222:    доступен только для белого списка / Tailscale"
